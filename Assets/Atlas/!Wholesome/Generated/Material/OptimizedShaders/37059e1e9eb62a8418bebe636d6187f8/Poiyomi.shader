Shader "Hidden/Locked/.poiyomi/Old Versions/8.1/Poiyomi Toon/37059e1e9eb62a8418bebe636d6187f8"
{
	Properties
	{
		...
		[sRGBWarning]_Rim2Mask ("Mask & Bias--{reference_properties:[_Rim2MaskPan, _Rim2MaskUV, _Rim2MaskChannel, _Rim2MaskInvert, _Rim2BiasIntensity], condition_showS:_Rim2Style==0}", 2D) = "white" { }
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_DISTORTIONFLOWTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 flowVector = POI2D_SAMPLER_PAN(_DistortionFlowTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DistortionFlowTextureUV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * 2 - 1;
				#else
				float4 flowVector = -1;
				#endif
				#if defined(PROP_DISTORTIONFLOWTEXTURE1) || !defined(OPTIMIZER_ENABLED)
			...
			void poiClearCoat(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float smoothness = (0.92 /*_ClearCoatSmoothness*/);
				float reflectionMask = (1.0 /*_ClearCoatReflectionStrength*/);
				float specularMask = (1.0 /*_ClearCoatSpecularStrength*/);
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PBRMaps = POI2D_SAMPLER_PAN_STOCHASTIC(_ClearCoatMaps, _MainTex, poiUV(poiMesh.uv[(2.0 /*_ClearCoatMapsUV*/)], _ClearCoatMaps_ST), float4(0,0,0,0), (0.0 /*_ClearCoatMapsStochastic*/));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiMesh.uv[(7.0 /*_MainTexUV*/)].xy;
